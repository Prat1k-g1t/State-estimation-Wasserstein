...
def dimension(I):
    ...
    """Must return 1 if for 1D problems."""
    #TODO: Clean to original
    return I.dim() - 2
...
def softmin_grid(eps, C_xy, h_y):
    ...
    D = dimension(h_y)
    #DEBUG:
    if D != 1: print('Dim of meas. in softmin {} and shape {}'.format(D, h_y.shape))
    #TODO: Clean to original
    if D == 1: 
    ...
    def softmin(a_log): # B,K,N,N,N
        ...
        #DEBUG:
        # print('a_log_j shape', a_log_j.shape)
        ...
        # print(kA_log_ij)
        # kA_log = kA_log_ij.logsumexp(dim=1)  # (B * N, N, 1)

        
        #TODO: Clean to original
        if D == 1:
            kA_log = kA_log_ij.logsumexp(dim=1)
            return kA_log.view(B, K, N)
        elif D == 2:
            kA_log = kA_log_ij.logsumexp(dim=2)
    ...
    if D == 1:
        ...
        # h_y = softmin(h_y[:,:,:,None])
        # h_y = softmin(h_y[:,:,None,:])
        # h_y = softmin(h_y.permute([0, 1, 2])).permute([0, 1, 2])  # Act on columns    

    elif D == 2:
        h_y = softmin(h_y)  # Act on lines 0,1,2,3
        h_y = softmin(h_y.permute([0, 1, 3, 2])).permute([0, 1, 3, 2])  # Act on columns

    elif D == 3:
        h_y = softmin(h_y)  # Act on dim 4 0,1,2,3,4
