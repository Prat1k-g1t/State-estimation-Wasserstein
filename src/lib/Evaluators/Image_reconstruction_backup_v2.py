...
def inf_stability_const(target: torch.Tensor, measures: torch.Tensor, 
    ...
    W2, evolution, bary1, bary2, wts = compute_barycenters(measures)
    ...
    for nfac in range(3):
        ...
        for iter in tqdm(range(niter)):        
            ...
            inf_loss = (torch.nn.MSELoss()(observation_bary1[None, None, :],
                                           observation_bary2[None, None, :]))
            ...
            inf_evolution['iter'].append(iter)
            inf_evolution['inf_sup_stability_const'].append(inf_loss.item())
            inf_evolution['sup_stability_const'].append(W2.item()/inf_loss.item())
...
class consensus_opt:
    def __init__(self, target: torch.Tensor, measures: torch.Tensor,
        ...
        conf = {'alpha': 50.0,
            'dt': 0.1,
            # 'max_time': 5.0,
            'sigma': 5.0, #self.k*(3.0+self.variance),
            'lamda': 1.0,
            # 'batch_args':{
            # 'batch_size':200,
            # 'batch_partial': False},
    ...
    def sup(self,x):
        ...
        observation_bary1 = torch.matmul(self.ker,self.bary1.squeeze())
        observation_bary2 = torch.matmul(self.ker,self.bary2.squeeze())
        ...
        print('L2.shape, loss.shape', L2.shape)
        return 1./L2.cpu().detach().numpy()
    ...
    def compute_barycenters(self):
        ...
        S = torch.arange(N,device=device)
        barycenter_evolution = {'loss':[],'weights1':[],'weights2':[]}
        
        tmp_loss = 1e20; tols = 1e-18

        for iter in tqdm(range(niter)):
        # for iter in range(niter):
            barycenter_evolution['weights1'].append(params[0].detach().clone())
            barycenter_evolution['weights2'].append(params[1].detach().clone())
            ...
            barycenter_evolution['loss'].append(loss.detach().item())
        ...
        self.bary1 = bary1.detach()
        self.bary2 = bary2.detach()
        
        return loss.detach().item(), barycenter_evolution
    ...
    def inf(self):
        ...
        W2, barycenter_evolution = self.compute_barycenters()
        #DEBUG:
        # print('barycenter loss', barycenter_evolution['loss'])
        # print('W2', W2)
        ...
        for nfac in range(3):
            ...
            inf_loss, tmp, sensors_history = self.compute_CBO(config, nfac, W2)
    ...
    def reconstruct_target_w_static_sensors(self, tstep, saved_dir):
                           
        params_opt = {'optimizer': 'Adam', 'lr': 0.01, 'nmax': 100,'type_loss':'W2', 'gamma':1}
        # frames_dir = check_create_dir(self.frames_dir+'/'+'reconstruct_target/')
        frames_dir = check_create_dir(saved_dir+'/'+'reconstruct_travelling_target_trial/t{}/'.format(tstep))
        ...
        for nfac in range(3):
            ...
            # sensor_locations = torch.linspace(field[0].item()*0.9, field[-1].item()*0.9, no_of_sensors).tolist(); arange = 'even'
            sensor_locations = torch.tensor(torch.load(saved_dir+'{}_sensor-coordinates'.format(no_of_sensors)), device=device)
            ...
            for iter in tqdm(range(niter)):
                ...
                #DEBUG:
                # print(observation_target.shape, observation_target.shape)
    ...
    def compute_CBO(self, config, nfac, W2):            
