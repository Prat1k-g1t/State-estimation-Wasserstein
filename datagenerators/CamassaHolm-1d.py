...
# -*- coding: utf-8 -*-
"""
Created on Sun May 21 01:05:17 2023
...
from .CH import Grid1d, SimulationCamassaHolm
...
def paramRange_1d():
    tmin = 0. ; tmax = 40.
    xmin = -10; xmax = 20 #Need to caliberate
    qmin = -2 ; qmax = 2 #q parameter, refer to Ehrlacher et al. 2020
    pmin = 0.2; pmax = 0.2 #p parameter, refer to Ehrlacher et al. 2020
    
    return [(xmin, xmax), (qmin, qmax), (pmin, pmax), (tmin,tmax)]

def solve(param,grid):
    param = param.numpy()
    q = param[1]; p = param[2]; t = param[3]; alpha = 1
    
    deltat = 0.1
    
    s = SimulationCamassaHolm(grid)
    s.init_cond(q, p, alpha)
    
    return s.evolve(deltat, t)
...
if __name__ == "__main__":
    ...
    for i in range(3):    
        ...
        # Save
        rootdir = check_create_dir(data_dir+'CamassaHolm1d/'+str(nparam)+'/'+str(i)+'/')
